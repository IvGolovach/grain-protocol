...
2. Verify local checks:
   ...
   - `python3 tools/ci/run_runner_suite.py --vectors-root conformance/vectors --runner-cmd cargo run -q --manifest-path core/rust/Cargo.toml -p grain-runner -- run --strict --vector --commit-sha "$(git rev-parse HEAD)" --out /tmp/suite-run.json`
