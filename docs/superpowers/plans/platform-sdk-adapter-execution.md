...
| 4 | Add iOS adapter pack: Keychain/file persistence boundary, trust provider, injected scanner flow, and scanner smoke hardening. | In progress | `codex/sdk-ios-adapter-pack` | Local strict SDK proof passed; PR/review pending |
...
## Current Step 4 Definition

Step 4 is complete when:

- The iOS scanner shell uses `trustAnchorID` plus `GrainTrustProvider` instead
  of raw trust material for production preview/accept paths.
- The iOS adapter pack provides a deterministic app-owned snapshot persistence
  boundary over `exportStoreSnapshot` / `restoreStoreSnapshot`; Keychain-ready
  APIs are isolated from protocol semantics and testable without device-only
  state.
- The injected camera flow proves scan preview, accept, duplicate accept,
...
- Scanner smoke checks assert no raw QR/COSE/DAG-CBOR/protocol-runner APIs, no
  hidden trust lookup, no network trust discovery, and no accepted-record write
  before verified accept.
