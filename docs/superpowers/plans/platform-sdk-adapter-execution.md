...
| 3 | Add production trust-provider surfaces across SDKs with explicit anchor IDs and no fallback trust. | In progress | `codex/sdk-trust-provider` | Local implementation and tests in progress |
...
## Current Step 3 Definition

Step 3 is complete when:

- Swift, Kotlin, and WASM wrappers expose explicit trust-provider surfaces that
  resolve a caller-supplied trust anchor ID to already-enrolled trust material.
- Missing or blank anchors fail closed with `SDK_ERR_TRUST_ANCHOR_REQUIRED`.
- Unknown anchors fail closed with `SDK_ERR_TRUST_ANCHOR_NOT_FOUND`.
- Provider-backed preview and accept fixtures pass through the public platform
  wrappers, with rejected accept paths proving no store mutation.
- CI has a platform trust-provider boundary guard that blocks hidden network
  lookup, default trust, TOFU, or fallback issuer patterns in generated SDK
  wrapper sources.
