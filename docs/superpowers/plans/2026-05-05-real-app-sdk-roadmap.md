...
| 6 | Custody and sync hardening | PR open | `codex/custody-sync-hardening-step6` | #54 | | `cargo test --manifest-path core/rust/Cargo.toml -p grain-client-core`; `scripts/sdk/check_generated_bindings.sh`; `scripts/sdk/check_swift_package.sh`; Kotlin SDK/example compile; WASM package check with rustup toolchain; docs/LLM/no-network/trust-boundary/secret-logging checks | Pending PR CI/review |
