...
| 5 | Android parity slice | Local validation | `codex/android-parity-slice-step5` | | | `cargo build --manifest-path core/rust/Cargo.toml -p grain-client-core`; Kotlin SDK/example `compileKotlin compileTestKotlin`; docs/LLM/no-network/trust-boundary/ledger checks | Pending PR CI/review; local full Kotlin/scanner smoke blocked by x86_64 JVM on arm64 host before repo checks |
