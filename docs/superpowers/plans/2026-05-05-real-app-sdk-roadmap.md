...
- 2026-05-06: Distribution and custody hardening recorded as local guidance for the next real-app layer. `docs/human/sdk/distribution-roadmap.md` defines source-only now, future SwiftPM/Maven/npm channels, and thin phone/glasses/robot UX responsibilities. `docs/human/sdk/custody-threat-model.md` defines app-owned custody boundaries, local trust bundles, snapshot export rules, and no secret logging expectations.
